...
### "Засыпание" Supabase
Supabase Free tier засыпает ровно через **7 дней** без подключений к БД. Если бот перестал отвечать:
1. Зайди в Supabase Dashboard → проект → **Resume project**
2. Перезапусти сервис на Render: **Manual Deploy → Deploy latest commit**

Чтобы избежать этого, настрой бесплатный внешний пинг (например, [UptimeRobot](https://uptimerobot.com)) на URL бота каждые 5 минут — это будет будить и Render, и Supabase.
